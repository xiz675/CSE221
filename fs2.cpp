...
static void fsRead(const uint32_t loopCount)
{
    ...
    for (const auto region : regions)
    {
        ...
        const auto testname = "fsread-seq-" + std::to_string(region);
    ...
    for (const auto region : regions)
    {
        ...
        const auto testname = "fsread-ran-" + std::to_string(region);
        PrintResult(testname, count, totalTime, 0, [&](const double perBlock)
        {
...
static const uint32_t DummyId = RegistTest("Filesystem Read", 'd', false, &fsRead, 1, false);
